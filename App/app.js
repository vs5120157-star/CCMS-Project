...
async function apiRequest(path, options = {}) {
    ...
    try {
        ...
        throw new Error('Backend server is not running. Start it with: cd backend && npm start');
    }
...
const getComplaints = () => JSON.parse(localStorage.getItem('rdecComplaints') || '[]');
const saveComplaints = (complaints) => localStorage.setItem('rdecComplaints', JSON.stringify(complaints));
...
function getCategories() {
    ...
    const savedCategories = JSON.parse(localStorage.getItem('rdecCategories') || 'null');
    if (!savedCategories) return defaultCategories;
    return [...savedCategories, ...defaultCategories.filter((defaultCategory) => !savedCategories.some((category) => category.name === defaultCategory.name))];
}
const saveCategories = (categories) => localStorage.setItem('rdecCategories', JSON.stringify(categories));
...
function getUsers() {
    ...
    if (student?.email && !users.some((user) => user.email === student.email)) users.push({ ...student, role: 'Student', status: 'Active' });
    if (admin?.email && !users.some((user) => user.email === admin.email)) users.push({ ...admin, name: 'Administrator', role: 'Admin', status: 'Active' });
    return users;
}
...
function getAdminProfile() {
    const admin = JSON.parse(localStorage.getItem('rdecAdmin') || 'null');
    return { name: admin?.name || 'Administrator', email: admin?.email || 'admin@example.com', role: admin?.role || 'Admin' };
}
...
if (adminProfileForm) {
    ...
    document.querySelector('#profile-role-badge').textContent = admin.role === 'Admin' ? 'Administrator' : admin.role;
    adminProfileForm.addEventListener('submit', (event) => {
        event.preventDefault();
        const updated = { name: adminProfileForm.name.value.trim(), email: adminProfileForm.email.value.trim(), role: adminProfileForm.role.value };
        ...
        document.querySelector('#profile-role-badge').textContent = updated.role === 'Admin' ? 'Administrator' : updated.role;
        document.querySelector('#admin-profile-message').textContent = 'Profile updated successfully.';
    });
}
...
function populateCategorySelects() {
    ...
    document.querySelectorAll('[data-category-select]').forEach((select) => {
        ...
        const placeholder = select.name === 'category' ? '<option value="" disabled selected>Select Category</option>' : `<option value="${select.id === 'manage-category-filter' ? 'All' : ''}">${select.id === 'manage-category-filter' ? 'Select Category' : 'Select Category'}</option>`;
        select.innerHTML = placeholder + categories.map((category) => `<option>${escapeHtml(category.name)}</option>`).join('');
        if (categories.some((category) => category.name === currentValue)) select.value = currentValue;
    });
}
...
if (loginForm) {
    ...
    loginForm.addEventListener('submit', async (event) => {
        ...
        try {
            const result = await apiRequest('/auth/login', { method: 'POST', body: JSON.stringify({ email, password, role: 'Student' }) });
...
if (registrationForm) {
    registrationForm.addEventListener('submit', async (event) => {
        ...
        if (password !== registrationForm.confirmPassword.value) {
            document.querySelector('#form-message').textContent = 'Passwords do not match.';
            return;
        }
        try {
            const result = await apiRequest('/auth/register', { method: 'POST', body: JSON.stringify({ fullName: name, email, phone: registrationForm.phone.value.trim(), rollNumber: registrationForm.rollNumber.value.trim(), password }) });
            localStorage.setItem('ccmsToken', result.token);
            localStorage.setItem('rdecStudent', JSON.stringify(result.user));
            navigateTo('Dashboard.html');
        } catch (error) {
...
if (adminRegistrationForm) {
    adminRegistrationForm.addEventListener('submit', async (event) => {
        ...
        try {
            const result = await apiRequest('/auth/register-admin', { method: 'POST', body: JSON.stringify({ name: form.name.value.trim(), email: form.email.value.trim(), phone: form.phone.value.trim(), password: form.password.value }) });
            localStorage.setItem('ccmsToken', result.token);
...
function renderComplaints() {
    ...
    if (complaintsBody) {
        complaintsBody.innerHTML = complaints.length ? complaints.map((complaint, index) => { const status = complaint.status || 'Pending'; return `<tr><td>${index + 1}</td><td>${escapeHtml(complaint.subject || '-')}</td><td>${escapeHtml(complaint.category || '-')}</td><td><span class="status ${statusClass(status)}">${escapeHtml(status)}</span></td><td>${escapeHtml(complaint.date || '-')}</td></tr>`; }).join('') : '<tr><td colspan="5">No complaints submitted yet.</td></tr>';
    }
    if (myComplaintsBody) {
        myComplaintsBody.innerHTML = complaints.length ? complaints.map((complaint, index) => { const status = complaint.status || 'Pending'; return `<tr><td>${index + 1}</td><td>${escapeHtml(complaint.subject || '-')}</td><td>${escapeHtml(complaint.category || '-')}</td><td><span class="status ${statusClass(status)}">${escapeHtml(status)}</span></td><td>${escapeHtml(complaint.date || '-')}</td><td><a href="#" class="view" data-index="${index}">View</a></td></tr>`; }).join('') : '<tr><td colspan="6">No complaints submitted yet.</td></tr>';
    }
    ...
    const counts = complaints.reduce((result, complaint) => { const status = complaint.status || 'Pending'; result[status] = (result[status] || 0) + 1; return result; }, {});
...
myComplaintsBody?.addEventListener('click', (event) => {
    ...
    if (!complaint) return;
    window.alert(`Subject: ${complaint.subject || '-'}\nCategory: ${complaint.category || '-'}\nStatus: ${complaint.status || 'Pending'}\n\n${complaint.description || '-'}`);
});
...
function renderAdminDashboard() {
    ...
    const complaints = getComplaints();
    const search = (document.querySelector('#admin-search')?.value || '').toLowerCase();
    const filter = document.querySelector('#status-filter')?.value || 'All';
    const filtered = complaints.filter((complaint) => {
        const text = `${complaint.subject} ${complaint.category} ${complaint.description}`.toLowerCase();
        return (!search || text.includes(search)) && (filter === 'All' || (complaint.status || 'Pending') === filter);
    });
    const counts = { total: complaints.length, pending: 0, progress: 0, resolved: 0 };
    ...
    adminComplaintsBody.innerHTML = filtered.length ? filtered.map((complaint) => {
    ...
    }).join('') : '<tr><td colspan="6" class="empty-state">No complaints found.</td></tr>';
}
...
document.querySelector('#refresh-complaints')?.addEventListener('click', renderComplaints);
...
// Categories page par category cards aur count render karta hai.
    ...
    if (summary) summary.textContent = `${categories.length} categor${categories.length === 1 ? 'y' : 'ies'}`;
    categoryList.innerHTML = categories.length ? categories.map((category, index) => {
        const complaintCount = complaints.filter((complaint) => complaint.category === category.name).length;
    ...
    }).join('') : '<div class="empty-state">No categories available.</div>';
}
...
function renderUsers() {
    ...
    const users = getUsers().filter((user) => {
        const text = `${user.name || ''} ${user.email || ''} ${user.phone || ''} ${user.rollNumber || ''}`.toLowerCase();
        return (!search || text.includes(search)) && (role === 'All' || user.role === role);
    });
    ...
    usersBody.innerHTML = users.length ? users.map((user, index) => {
        ...
        const meta = user.rollNumber ? `Roll No: ${escapeHtml(user.rollNumber)}` : 'Registered account';
...
// Manage Complaints page ke search aur filters ke hisaab se rows render karta hai.
    ...
    const complaints = getComplaints();
    const search = (document.querySelector('#manage-search')?.value || '').trim().toLowerCase();
    const statusFilter = document.querySelector('#manage-status-filter')?.value || 'All';
    const categoryFilter = document.querySelector('#manage-category-filter')?.value || 'All';
    const filtered = complaints.map((complaint, index) => ({ complaint, index })).filter(({ complaint }) => {
        const text = `${complaint.subject || ''} ${complaint.category || ''} ${complaint.description || ''}`.toLowerCase();
        const status = complaint.status || 'Pending';
        return (!search || text.includes(search)) && (statusFilter === 'All' || status === statusFilter) && (categoryFilter === 'All' || complaint.category === categoryFilter);
    });
    manageComplaintsBody.innerHTML = filtered.length ? filtered.map(({ complaint, index }, rowIndex) => {
        const status = complaint.status || 'Pending';
        const statusClass = status === 'In Progress' ? 'progress' : status.toLowerCase();
    ...
    }).join('') : '<tr><td colspan="7" class="empty-state">No complaints found.</td></tr>';
}

renderManageComplaints();
// Manage Complaints ke search/filter controls ko table rendering se connect karta hai.
document.querySelector('#manage-search-button')?.addEventListener('click', renderManageComplaints);
document.querySelector('#manage-search')?.addEventListener('keydown', (event) => {
    if (event.key === 'Enter') renderManageComplaints();
});
document.querySelector('#manage-status-filter')?.addEventListener('change', renderManageComplaints);
document.querySelector('#manage-category-filter')?.addEventListener('change', renderManageComplaints);
...
manageComplaintsBody?.addEventListener('change', async (event) => {
    ...
    try {
        await apiRequest(`/complaints/${complaints[index]._id}/status`, { method: 'PATCH', body: JSON.stringify({ status: event.target.value }) });
    } catch (error) {
...
function renderReports() {
    ...
    const selectedStatus = document.querySelector('#report-status')?.value || 'All';
    const selectedCategory = document.querySelector('#report-category')?.value || 'All';
    const categories = [...new Set(allComplaints.map((complaint) => complaint.category).filter(Boolean))].sort();
    const categorySelect = document.querySelector('#report-category');
    if (categorySelect && categorySelect.options.length !== categories.length + 1) {
    ...
    const complaints = allComplaints.filter((complaint) => (selectedStatus === 'All' || (complaint.status || 'Pending') === selectedStatus) && (selectedCategory === 'All' || complaint.category === selectedCategory));
    const counts = { Pending: 0, 'In Progress': 0, Resolved: 0 };
    allComplaints.forEach((complaint) => { counts[complaint.status || 'Pending'] = (counts[complaint.status || 'Pending'] || 0) + 1; });
    ...
    document.querySelector('#report-progress').textContent = counts['In Progress'];
    document.querySelector('#report-resolved').textContent = counts.Resolved;
    const renderBars = (items, selector) => {
        const max = Math.max(...items.map((item) => item.count), 1);
        document.querySelector(selector).innerHTML = items.map((item) => `<div class="bar-row"><div class="bar-label"><span>${escapeHtml(item.label)}</span><strong>${item.count}</strong></div><div class="bar-track"><div class="bar-fill" style="width:${item.count / max * 100}%"></div></div></div>`).join('') || '<div class="empty-state">No data available.</div>';
    };
    renderBars([{ label: 'Pending', count: counts.Pending }, { label: 'In Progress', count: counts['In Progress'] }, { label: 'Resolved', count: counts.Resolved }], '#status-bars');
    const categoryCounts = categories.map((category) => ({ label: category, count: allComplaints.filter((complaint) => complaint.category === category).length })).sort((a, b) => b.count - a.count);
    renderBars(categoryCounts, '#category-bars');
    document.querySelector('#report-result-count').textContent = `${complaints.length} result${complaints.length === 1 ? '' : 's'}`;
    reportTableBody.innerHTML = complaints.length ? complaints.map((complaint, index) => { const status = complaint.status || 'Pending'; return `<tr><td>${index + 1}</td><td>${escapeHtml(complaint.subject || '-')}</td><td>${escapeHtml(complaint.category || '-')}</td><td><span class="report-status ${status.toLowerCase().replace(' ', '-')}">${escapeHtml(status)}</span></td><td>${escapeHtml(complaint.date || '-')}</td></tr>`; }).join('') : '<tr><td colspan="5" class="empty-state">No complaints match the selected filters.</td></tr>';
}

renderReports();
// Reports filters aur refresh button report ko dobara calculate karte hain.
document.querySelector('#report-status')?.addEventListener('change', renderReports);
document.querySelector('#report-category')?.addEventListener('change', renderReports);
document.querySelector('#refresh-report')?.addEventListener('click', renderReports);
...
categoryForm?.addEventListener('submit', (event) => {
    ...
    if (categories.some((category) => category.name.toLowerCase() === name.toLowerCase())) {
        document.querySelector('#category-message').textContent = 'This category already exists.';
        return;
    }
    apiRequest('/categories', { method: 'POST', body: JSON.stringify({ name, description }) }).then((category) => {
        categories.push(category);
        saveCategories(categories);
        categoryForm.reset();
        document.querySelector('#category-message').textContent = 'Category added successfully.';
        renderCategories();
        populateCategorySelects();
    }).catch((error) => { document.querySelector('#category-message').textContent = error.message; });
});
...
categoryList?.addEventListener('click', async (event) => {
    ...
    if (categoryId) await apiRequest(`/categories/${categoryId}`, { method: 'DELETE' });
...
document.querySelector('#admin-search')?.addEventListener('input', renderAdminDashboard);
document.querySelector('#status-filter')?.addEventListener('change', renderAdminDashboard);
...
adminComplaintsBody?.addEventListener('change', async (event) => {
    ...
    try {
        await apiRequest(`/complaints/${complaint._id}/status`, { method: 'PATCH', body: JSON.stringify({ status: event.target.value }) });
    } catch (error) {
        console.error(error.message);
    }
    saveComplaints(complaints);
...
if (complaintForm) {
    complaintForm.addEventListener('submit', async (event) => {
        ...
        try {
            await apiRequest('/complaints', { method: 'POST', body: JSON.stringify({ subject: formData.get('subject'), category: formData.get('category'), description: formData.get('description') }) });
            await syncComplaints();
        } catch (error) {
            document.querySelector('#form-message')?.replaceChildren(document.createTextNode(error.message));
            return;
        }
        complaintForm.reset();
...
document.querySelectorAll('a[href="#logout"]').forEach((logout) => logout.addEventListener('click', (event) => {
    event.preventDefault();
